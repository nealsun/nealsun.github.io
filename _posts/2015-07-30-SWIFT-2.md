...
swift分别使用关键字`let`和`var`来声明常量和变量，变量类型的声明跟在名称后面用`：`分隔。 不过swift引入了类型推断，当编译器可以推断出变量的类型时，不需要显示的声明变量类型。

``` swift
let aConst = 70.0 //double，编译器根据字面量70.0推断类型是double
var aVar = 30 //int，编译器根据字面量30推断类型是int
let aDoubleConst: Double = 70 //double， 需要显示声明，因为赋了int的字面量，编译器会推断为整型
var aStringVar: String //string，需要显示声明，因为只声明了变量名，编译器缺少推断信息
```
...
swift的`Array`和`Dictionary`是有类型的，如果一个`Array`声明为一个`String`类型的`Array`，那么就不能给他增加其他类型的变量，而objc的`NSArray`可以添加任何对象，swift为了兼容这一点引入了几个特殊类型，`AnyObject`和`Any`，使得`[AnyObject]`能添加任何对象。这样真的大丈夫？为了兼容损失了一丢丢的优雅？
...
如果函数体只有一行不需要显示`return`返回值，那句唯一的执行语句就会作为返回值。
...
如上代码所示，子类覆盖父类必须用`override`关键字显示声明，否则会报错。这也体现了swift编译器的严谨，防止写代码时覆盖父类的方法。
...
可以使用`where`来限制泛型类型的要求，如遵守某种协议，或两个泛型是否是同一个类型等。在简单情况下，可以省略`where`。
